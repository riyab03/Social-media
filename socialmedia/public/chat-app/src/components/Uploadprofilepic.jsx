       ...
       <div onMouseEnter={() => setIsHovered(true)} onMouseLeave={() => setIsHovered(false)} >
        <label htmlFor="profilepicupload" >
         <div style={{border:'2px solid #bebdbe',width:`${isMobile?'250px':'300px'}`,height:`${isMobile?'250px':'300px'}`,borderRadius:'50%',position:'relative',cursor:'pointer'}}>
          {profilePic?<img src={profilePic} alt='profile' style={{filter:`${isHovered?'grayscale(2)':'none'}`,width:`${isMobile?'250px':'300px'}`,height:`${isMobile?'250px':'300px'}`,borderRadius:'50%'}} />:<Avatar sx={{width:`${isMobile?'250px':'300px'}`,height:`${isMobile?'250px':'300px'}`,borderRadius:'50%',background:purple[500]}} variant="rounded">{group&&<People></People>}</Avatar>}
