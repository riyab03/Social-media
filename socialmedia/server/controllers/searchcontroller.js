...
module.exports.showwholeprofile=async(req,res,next)=>{
  let user=await User.findOne({_id:req.params.id});
  let allposts=await posts.find({creator:req.param.id});
  res.json({profileImage:user.profileImage,username:user.username});
}